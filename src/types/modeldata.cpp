...
void ModelData::Draw( VkCommandBuffer c, uint32_t i )
{
	...
	for ( uint32_t j = 0; j < aMeshes.GetSize(  ); ++j )
	{
		...
		// VkDescriptorSet sets[  ] = { mesh.apTexture->aSets[ i ], mesh.GetUniformData().aSets[ i ] };
		VkDescriptorSet sets[  ] = { mesh.apTexture->aSets[ i ], aUniformData.aSets[ i ] };
...
void ModelData::FreeOldResources(  )
{
	...
	//for ( uint32_t j = 0; j < aMeshes.GetSize(  ); ++j )
	{
		...
		{
			/* Free uniform data.  */
			UniformDescriptor& uniformdata = GetUniformData();
			vkDestroyBuffer( DEVICE, uniformdata.aData[i], NULL );
			vkFreeMemory( DEVICE, uniformdata.aMem[i], NULL );

			/* Free uniform layout. */
			vkDestroyDescriptorSetLayout( DEVICE, GetUniformLayout(), NULL );
		}
	}
}
