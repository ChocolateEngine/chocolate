...
VkSurfaceCapabilitiesKHR   g_surface_capabilities{};
...
u32 vk_get_memory_type( u32 type_filter, VkMemoryPropertyFlags properties )
{
	...
	vkGetPhysicalDeviceMemoryProperties( g_vk_physical_device, &memProperties );
...
{
	u32 image_count = g_surface_capabilities.minImageCount;

	if ( g_surface_capabilities.maxImageCount > 0 && image_count > g_surface_capabilities.maxImageCount )
		image_count = g_surface_capabilities.maxImageCount;
...
VkSurfaceFormatKHR vk_swapchain_choose_surface_format()
{
	for ( u32 i = 0; i < g_surf_format_count; i++ )
	{
		...
		{
			return g_surf_formats[ i ];
		}
	}

	return g_surf_formats[ 0 ];
}
...
VkExtent2D vk_swapchain_extent( r_window_data_t* window )
{
	...
	VkExtent2D size{
		std::max( g_surface_capabilities.minImageExtent.width, std::min( g_surface_capabilities.maxImageExtent.width, (u32)width ) ),
		std::max( g_surface_capabilities.minImageExtent.height, std::min( g_surface_capabilities.maxImageExtent.height, (u32)height ) ),
	};
...
void vk_swapchain_update_info( VkSurfaceKHR surface )
{
	...
	vk_check( vkGetPhysicalDeviceSurfaceCapabilitiesKHR( g_vk_physical_device, surface, &g_surface_capabilities ),
...
{
	g_surface_capabilities = info.surface_capabilities;
...
bool vk_swapchain_create( r_window_data_t* window, VkSwapchainKHR old_swapchain )
{
	...
	window->swap_surface_format = vk_swapchain_choose_surface_format();
	window->swap_present_mode   = vk_swapchain_choose_present_mode();
	...
	createInfo.imageFormat      = window->swap_surface_format.format;
	createInfo.imageColorSpace  = window->swap_surface_format.colorSpace;
	createInfo.imageExtent      = window->swap_extent;
	createInfo.imageArrayLayers = 1;
	createInfo.imageUsage       = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;
	createInfo.preTransform     = g_surface_capabilities.currentTransform;
	createInfo.compositeAlpha   = VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR;
	createInfo.presentMode      = window->swap_present_mode;
	...
	{
		queueFamilyIndices[ 0 ]          = g_queue_family_graphics;
		queueFamilyIndices[ 1 ]          = g_queue_family_transfer;
	...
	for ( u32 i = 0; i < image_count; ++i )
	{
		...
		view_info.format                          = g_color_format;
		...
		vk_check( vkCreateImageView( g_vk_device, &view_info, nullptr, &window->swap_image_views[ i ] ), "Failed to create image view" );
