...
void Basic3D::UpdateBuffers( uint32_t sCurrentImage, BaseRenderable* spRenderable )
{
	...
	void* data;
	vkMapMemory( DEVICE, materialsystem->GetUniformData( spRenderable->GetID() ).aMem[ sCurrentImage ], 0, sizeof( ubo ), 0, &data );
	memcpy( data, &ubo, sizeof( ubo ) );
	vkUnmapMemory( DEVICE, materialsystem->GetUniformData( spRenderable->GetID() ).aMem[ sCurrentImage ] );
}
